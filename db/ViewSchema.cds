...
entity VERUSER as select 
    key ID,
        ...
        RETIRO,
        createdAt,
        TO_INTEGER(MONTHS_BETWEEN(TO_DATE(INICIO, 'YYYY-MM-DD'), (CASE WHEN RETIRO IS NULL THEN CURRENT_DATE ELSE RETIRO END) ) / 12) AS ANTIGUEDADA: Integer,
        MOD(MONTHS_BETWEEN(TO_DATE(INICIO, 'YYYY-MM-DD'), (CASE WHEN RETIRO IS NULL THEN CURRENT_DATE ELSE RETIRO END) ), 12) AS ANTIGUEDADM: Integer,
...
entity VERPERFIL as select 
       ...
       p.CARGO.ID as cargoId,
	   c.NOMBRE_CARGO as cargo,
	   c.OBJETIVO as objCargo,
       p.PROCESO.ID as procesoId, 
